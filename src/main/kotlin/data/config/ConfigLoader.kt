...
object ConfigLoader {
    ...
    fun load(): AggregatorConfig {
        ...
        fun csv(key: String, default: List<String>) =
            p.getProperty(key)?.split(',')?.map { it.trim() }?.filter { it.isNotEmpty() } ?: default

        return AggregatorConfig(
            documentKeywords = csv("document.keywords", listOf("ใบส่งสินค้า", "invoice")),
